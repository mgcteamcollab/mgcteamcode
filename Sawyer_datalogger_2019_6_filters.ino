...
//  recordData();   // records data every minute
  prevMin = currentMin;

// records data at specific intervals. 
if ((currentMin == 4 || currentMin == 9 || currentMin == 14 || currentMin == 19|| currentMin == 24 || currentMin == 29 || currentMin == 34|| currentMin == 39 || currentMin == 44 || currentMin == 49|| currentMin == 54 || currentMin == 59) && (true)) {
    recordData();  
    }
...
//    {
      ...
      tNow = rtc.now();
      dataToWrite.concat(String(microI2CAdresses[1]));    
      dataToWrite.concat(",");
      ...
      Wire.requestFrom(microI2CAdresses[1], numBytesRequested);
...
//      SD.remove("datalog.csv");   // gets rid of the file before creating it fresh
